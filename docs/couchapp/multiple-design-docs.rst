...
I want to setup a project for a new database ``test6`` with a design doc
called ``design_doc1``.
...
.. note::
    It looks like couchapp doesn't pick up the default db in what
...
Now push ``design_doc1``.
Note that I have to include the url of the database as a parameter.
Couchapp doesn't seem to pick up the default db
...
You should see ``_design/design_doc1`` listed on the html page. 
That's good, it means that ``design_doc1`` was created.
...
Couchapp generate had created a sample view called recent-items.
Try querying it::
