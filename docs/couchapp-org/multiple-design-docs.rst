...
Here is what I did to use couchapp with multiple design documents. I
want to setup a project for a new database test6 with a design doc
called design\_doc1

Make sure couchdb and couchapp are installed and that couchdb is
started.
...
On my machine (Debian squeeze), couchapp seems to generate the directory
with owner root. Change owner to john
...
NOTE: It looks like couchapp doesn't pick up the default db in what
follows when I do couchapp push
...
Make a directory for design\_doc1

::

    $ mkdir _design/design_doc1

move the design doc files created with couchapp generate to the
design\_doc1 directory:
...
now push design\_doc1. Note that I have to include the url of the
database as a parameter. Couchapp doesn't seem to pick up the default db
when I push from the \_design directory.
...
You should see "\_design/design\_doc1" listed on the html page. That's
good, it means that design\_doc1 was created.

Take a look at design\_doc1 in the futon web admin. Open this URL in
your browser:
...
You should see a nice listing of the design\_doc1. Try opening the index
...
test6/\_design/design\_doc1/\_attachments/index.html

Couchapp generate had created a sample view called recent-items. Try
querying it:
...
That's it. Multiple design can be used to create different interfaces
for users with different roles. For example, consider some data and the
different ways that and admin versus a regular user interacting with it.
