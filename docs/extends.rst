...
-  hooks: allows you to add actions on pre-/post
   (push, clone, pushdocs, pushapps) events.
...
Eggs uri are entry points uri starting with ``egg:`` prefix.
...
:scheme:            list of url prefix on which this handler will be use.
                    (e.g.: ['git', 'git+ssh'] for git://\|git/ssh:// urls)
